    ...
    for line in text:
        ...
        for word in line.split():
            ...
            if word in linis_dict:
                ...
                words_cnt += 1 # не учитываем стоп-слова
       # X = np.append(X, sentiment_sum) #суммарный сантимент

            # Усредненный сантимент
            if words_cnt != 0:
                sentiment_average = sentiment_sum / words_cnt

        X = np.append(X, sentiment_average)

    print(X)
#f.writelines(line + "\t" + str(sentiment_sum))
...
if __name__ == '__main__':
    summarize('sentiment_average.xml')
