...
def get_linis_dictionary():
    ...
    with open("words_all_full_rating.csv", "r") as f:
        ...
        for row in reader:
            linis[row[0]] = abs(int(row[3])) #берем по модулю
    print(linis)
    return linis
...
def get_sentiment_indices(text):
    ...
    for line in text:
        ...
        for word in line.split():
            ...
            if word in linis_dict:
                sentiment_sum += linis_dict[word]

        #print(sentiment_sum)


        X = np.append(X, sentiment_sum) #суммарный сантимент
        '''
            # Усредненный сантимент
            sentiment_average.py = 0
            if words_cnt != 0:
                sentiment_average.py = sentiment_sum / words_cnt
        
            X = np.append(X, sentiment_average.py)
        '''
    print(X)
    # with open("sentiment.txt", 'w', encoding='utf8') as f:
    #     for i in range(X.size):
    #         print(X.item(i), "\t", X.item(i+1), "\n")
    #         i += 2
